...
function updateBestScore() {
  bestScoreArray.forEach((score, index) => {
    // Select correct Best Score to update (10, 25, 50 or 99)
...
function createEquations() {
  ...
  for (let i = 0; i < wrongEquations; i++) {
    ...
    secondNumber = getRandomInt(9);
    const equationValue = firstNumber * secondNumber;
    // I create 3 different wrong equations and I choose one of them to display
    wrongFormat[0] = `${firstNumber} x ${secondNumber + 1} = ${equationValue}`;
    wrongFormat[1] = `${firstNumber} x ${secondNumber} = ${equationValue + 1}`;
    wrongFormat[2] = `${firstNumber + 1} x ${secondNumber} = ${equationValue}`;
    const formatChoice = (firstNumber === 0 || secondNumber === 0) ? 1 : getRandomInt(3);
...
function selectQuestionAmount(ev) {
  ev.preventDefault();
...
startForm.addEventListener('click', () => {
  radioContainers.forEach( radioEl => {
    ...
    if (radioEl.children[1].checked) {            // radioEl.children[1] targets the 2nd children of the radio-container div, which is the radio input
